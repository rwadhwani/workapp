...
sub getIncome {
    ...
    if ($workdate_from eq $workdate_to) {
        ...
        foreach (0..$date_diff) {
            if ($alert) {
                $errmsg .= "[$date_diff] select workdate, total_hours, bonus from rota where YEAR(workdate)=YEAR('$workdate_from') AND MONTH(workdate)=MONTH('$workdate_from') AND DAY(workdate)=DAY('$workdate_from')+$_ and employer=\"$employer\" limit 1<br />";
            }
            $rquery = $dbh->prepare("select workdate, total_hours, bonus from rota where YEAR(workdate)=YEAR('$workdate_from') AND MONTH(workdate)=MONTH('$workdate_from') AND DAY(workdate)=DAY('$workdate_from')+$_ and employer=\"$employer\" limit 1");        # limit 1 and no while loop = assuming you work only ONCE in a day (not twice e.g., 0900-1200 then again 1800-2100)
            $rquery->execute();
            $rdata = $rquery->fetchrow_hashref;
