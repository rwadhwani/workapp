...
# Author            :   Rajesh Wadhwani, Lavina Wadhwani
# Last modified     :   30-Nov-2013
# Version history   :   See version_history.txt
#
...
# NEXT
$query = $dbh->prepare("select MIN(workdate) as workdate_from, MAX(workdate) as workdate_to, count(workdate) as total_days, SUM(total_hours) as total_hours from rota where MONTH(workdate)=MONTH(CURRENT_DATE) and YEAR(workdate)=YEAR(CURRENT_DATE) and workdate <= CURRENT_DATE and employer=\"$current_employer\"");
$query->execute();
$data = $query->fetchrow_hashref;
...
# NEXT
$query = $dbh->prepare("select MIN(workdate) as workdate_from, MAX(workdate) as workdate_to, count(workdate) as total_days, SUM(total_hours) as total_hours from rota where MONTH(workdate)=MONTH(CURRENT_DATE) and YEAR(workdate)=YEAR(CURRENT_DATE) and employer=\"$current_employer\"");
$query->execute();
$data = $query->fetchrow_hashref;
if ($$data{total_days} eq "0") { $total_days = "<span class=\"red\">0 days</span>"; }
else { $total_days = "$$data{total_days} days"; }
...
# NEXT
$query = $dbh->prepare("select SUM(total_hours) as total_hours, MIN(workdate) as workdate_from, MAX(workdate) as workdate_to, SUM(bonus) as bonus from rota where YEAR(workdate)=YEAR(CURRENT_DATE) and employer=\"$current_employer\"");
$query->execute();
$data = $query->fetchrow_hashref;
$current_year_pay = &getIncome("$$data{workdate_from}", "$$data{workdate_to}", "$$data{total_hours}", "");
if ($current_year_pay > 0) { $content =~ s|<!--INCOME_CURRENT_YEAR-->|$$sdata{currency}$current_year_pay &nbsp;($$sdata{currency}$$data{bonus} bonus)|g; }
else { $content =~ s|<!--INCOME_CURRENT_YEAR-->|<span class="red">$$sdata{currency}$current_year_pay</span>|g; }

# NEXT
$query = $dbh->prepare("select SUM(total_hours) as total_hours, MIN(workdate) as workdate_from, MAX(workdate) as workdate_to, SUM(bonus) as bonus from rota where YEAR(workdate)=YEAR(CURRENT_DATE)-1 and employer=\"$current_employer\"");
$query->execute();
$data = $query->fetchrow_hashref;
$last_year_pay = &getIncome("$$data{workdate_from}", "$$data{workdate_to}", "$$data{total_hours}");
if ($last_year_pay > 0) { $content =~ s|<!--INCOME_LAST_YEAR-->|$$sdata{currency}$last_year_pay &nbsp;($$sdata{currency}$$data{bonus} bonus)|; }
...
# NEXT
$query = $dbh->prepare("select SUM(directories) as total_directories, SUM(bonus) as bonus from rota where employer=\"$current_employer\"");
$query->execute();
$data = $query->fetchrow_hashref;
if ($$data{bonus} > 0) { $content =~ s|<!--TOTAL_BONUS-->|$$sdata{currency}$$data{bonus}|; }
else { $content =~ s|<!--TOTAL_BONUS-->|<span class="red">$$sdata{currency}0</span>|; }
    ...
    if ($workdate_from eq $workdate_to) {
        ...
        foreach (0..$date_diff) {
            ...
            if ($$rdata{workdate} ne "") {                          # if worked on 'this' date, then get 'rate' from account_settings
                ...
                $iquery->execute();
                $idata = $iquery->fetchrow_hashref;
                #if ($$rdata{break_time} > 0) { $$rdata{total_hours} = $$rdata{total_hours} - $$rdata{break_time}; }        # reduce break_time from total_hours prior to getting total payment for 'this' date, however commented this as it's more efficient to perform this action on 'Manage rota' when adding/updating Rota
